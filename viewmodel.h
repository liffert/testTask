...
public:
    ...
    const QModelIndex getItemIndex(const int row, const int column = 0);
    const QStandardItem *getItem(const QModelIndex &index) const;
    void search(const QString &arg1, bool onlyFavorite);
    bool favoritePressed(const QModelIndex &index, bool onlyFavorite);
    void call(const QModelIndex &index);
    void favoriteFilter(const bool onlyFavorite, const QString &searchLineText);
    bool isFavoriteItem(const QModelIndex &index);
    QModelIndex jumpTo(const char letter);
